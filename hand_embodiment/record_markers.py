...
class MarkerBasedRecordMapping(TimeableMixin):
    ...
    hand_state : mocap.mano.HandState, optional (default: None)
        If there is already a hand state object, this can be 1, -9.6733e-02, 2.5803e-01,reused for the
...
class ManoFingerKinematics:
    ...
    def reduce_pose_parameters(self, hand_state):
        ...
        t=hand_state.pose_parameters["hands_mean"][self.finger_pose_param_indices]
        global_orient_mean = np.zeros([3])
        t=np.concatenate([global_orient_mean,t],axis=0)
