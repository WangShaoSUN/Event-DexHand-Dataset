...
class HandState:
    ...
    def __init__(self, left=True):
        ...
        model_parameters['hands_mean']=np.array(model_parameters['hands_mean'])
        global_orient_mean = np.zeros([3])
        model_parameters['hands_mean']=np.concatenate([global_orient_mean, model_parameters["hands_mean"]], axis=0)
        #self.pose = np.concatenate([global_orient_mean, model_parameters["hands_mean"]], axis=0) + self.pose
        self.pose_parameters = {
            ...
            "hands_mean":model_parameters['hands_mean']
        }
...
def hand_vertices(J, weights, kintree_table, v_template, posedirs,hands_mean, pose=None):
    ...
    temp_pose=pose.copy()
    temp_pose=temp_pose+hands_mean
    temp_pose =temp_pose.reshape(-1, 3)
    v_posed = v_template + posedirs.dot(lrotmin(temp_pose))#计算v_shaped,即文章中的T+BsT+BpT,此处手的形状大小T+BsT=——template
    vertices = forward_kinematic(temp_pose, v_posed, J, weights, kintree_table)
    return vertices
