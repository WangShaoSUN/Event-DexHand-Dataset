...
class HandEmbodiment(TimeableMixin):
    ...
    def _mano_forward_kinematics(self, use_cached_forward_kinematics):
        ...
        i=0
        m=[]
        # markers_in_handbase_all=np.load('/home/ustc/code/hand_embodiment-main/verts.npy')
        print(self.finger_names_)
        for finger_name in self.finger_names_:
            if use_cached_forward_kinematics:
                ...
                #markers_in_handbase=markers_in_handbase_all[:,i,:]




            markers_in_robotbase = pt.transform(
                self.handbase2robotbase,#markers_in_handbase)[:,:3]
                pt.vectors_to_points(markers_in_handbase))[:, :3]
            n_ee_frames = len(self.target_finger_chains[finger_name].ee_frames)

            desired_positions[finger_name] = markers_in_robotbase[:n_ee_frames]#markers_in_handbase_all[:,i,:]
            m.append(desired_positions[finger_name])
            i = i + 1
        m=np.asarray(m)
        np.save('desire_emb.npy', m)
