    ...
    with torch.no_grad():
        for iteration, (frames, meta_data) in enumerate(val_dataloader_normal):
            ...
            for step in range(steps):
                ...
                # print("joints:")
                # print(pred_3d_joints_sub)
                # print(gt_3d_joints_sub)
        ...
        for iteration, (frames, meta_data) in enumerate(val_dataloader_fast):
            ...
            print(
                meta_data[-1]['mano']['hand_pose'][0].numpy())

            # plt.imshow(frames[-1]['rgb'][0].numpy())
            # plt.show()
            # print(meta_data[4]['mano']['hand_pose'][0])
            # cv2.imshow("image",frames[-1]['rgb'][0].numpy())
            # cv2.waitKey()
...
def get_config():
    ...
    parser.add_argument('--resume_checkpoint', type=str, default='EvImHandNet.pth')
    parser.add_argument('--config_merge', type=str, default='src/configs/config/eval_temporal.yaml')
    parser.add_argument('--run_eval_only', action='store_true',default=True)
